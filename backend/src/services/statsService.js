...
export const getTodayStats = async (locationId) => {
    ...
    let query = `
        SELECT 
            ...
            (SELECT ISNULL(SUM(CASE WHEN Ord_Type = 'TO' AND bill_valid != 'X' THEN No_Of_Pax ELSE 0 END), 0) FROM bill_header WHERE CAST(bill_date AS DATE) = @today ${locFilter}) as guest_count,
